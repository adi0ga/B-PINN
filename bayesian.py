    ...
    def build_posterior(self, bnn_fn):
        ...
        def _fn(*variables):
           ...
           # y_u_pred = _fn(x_u)
            ...
            bnd_likeli_1=tfd.Normal(loc=self.y0, scale=self.noise_u * 1)
            with tf.GradientTape() as g_t:
                g_t.watch(x_u)
                y_u_pred=_fn(x_u)
            u_t = g_t.gradient(y_u_pred, x_u)
            bnd_likeli_2=tfd.Normal(loc=self.ydash0, scale=self.noise_u * 1)
            ...
            log_likeli = (
                tf.reduce_sum(u_likeli.log_prob(u_t))
                +1000*tf.reduce_sum(bnd_likeli_1.log_prob(y_u_pred[0]))
                +1000*tf.reduce_sum(bnd_likeli_2.log_prob(u_t[0]))
